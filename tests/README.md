...
| Pacing for saved September plans | Skipped until the same saved plan is exported from Performance Pacing on the live app and on the test link. `python tools/compare_pacing.py LIVE.xlsx TEST.xlsx` then compares every plan spend and plan applications figure and the plan name, and reports MATCH or each difference. Exports hold RAC spend figures, so they stay in the data folder. |
