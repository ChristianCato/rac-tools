...
(function (RAC) {
  ...
  function method(A, role, plan, backtest) {
    ...
    add('What the plan does',
      `The plan starts from the monthly budget, which includes platform fees. Indeed Premium (campaigns x days in the month x ${f.gbp(v.premiumRate)} a day, plus the Indeed fee where fees apply) and the Combined Activity reserve come off the top${v.includesDisplay ? ', and the Google Display remarketing campaign is part of Combined Activity, so its spend sits in that reserve rather than in the planned Google spend' : ''}. What remains is the deployable budget.`,
